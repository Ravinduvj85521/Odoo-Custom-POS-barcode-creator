...
class PrintProductLabel(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if 'label_type_id' in fields_list and not default_vals.get('label_type_id'):
            default_vals['label_type_id'] = self.env.ref('custom_product_label.type_product').id
        # Auto-fill pricelist when opened from a pricelist record
        if self.env.context.get('default_pricelist_id'):
            default_vals['pricelist_id'] = self.env.context['default_pricelist_id']
        return default_vals
    ...
    pricelist_id = fields.Many2one(
        ...
        string='Pricelist',
        help='Select a pricelist to print its price on the labels instead of the standard sales price.',
    )

    @api.depends('report_id')
    def _compute_is_template_report(self):
        for wizard in self:
            # Template report is not used in this module
            wizard.is_template_report = False
    ...
    def get_quick_report_action(
        ...
        report = self.env.user.print_label_report_id \
                 or self.env.ref('custom_product_label.action_report_product_label_50x38')
